...
- **Build the merged Dashboard (v2.5.0):** copy `templates/build-dashboards.js` to `Agent-System/build-dashboards.js` and run it (`node build-dashboards.js`, prints `Built: …`). It rebuilds `dashboard-pinned.html` with a **Daily Hub injected into the Today tab** (▶ Run-now menu + Apply/Accept/Reject actions with a copy-line), and writes `daily-todo.html`/`daily-menu.html` as small "moved into the Dashboard" pointer pages. Register/refresh the Cowork artifact `create_artifact` id **`dashboard`** (html_path → `Agent-System/dashboard-pinned.html`). Preserves all data — it only READS the queues. If the truncation warning fires, Read the named files with the Read tool, Write clean copies to a temp dir, and re-run with `--src <that dir>`.
...
- **Daily refresh task:** ensure `refresh-pinned-dashboards` exists (create it ~30–45 min after Coach if missing, `notifyOnCompletion:false`). Its job now: run `node build-dashboards.js` then `update_artifact` id **`dashboard`** only. If an older version's prompt also updated `daily-todo`, rewrite it to the dashboard-only version.
