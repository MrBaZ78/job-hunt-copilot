...
- Build the **Daily To-Do panel + pin the dashboard** (new in v2.4.0): copy `templates/build-dashboards.js` to `Agent-System/build-dashboards.js`, run it in the shell (`node build-dashboards.js`, prints `Built: …`), then register/refresh two Cowork artifacts — `create_artifact` id **`daily-todo`** (html_path → `Agent-System/daily-todo.html`) and id **`dashboard`** (html_path → `Agent-System/dashboard-pinned.html`). This gives existing users the tap-to-act Apply/Not-interested + Accept/Reject panel and the pinned dashboard, alongside their Daily Menu. Preserves all data — it only READS the queues. If the shell reports a truncated queue file, Read it with the Read tool, Write a clean copy to a temp dir, and re-run with `--src <that dir>`.
- Create the **daily refresh** task if it doesn't exist: a scheduled task `refresh-pinned-dashboards` (~30–45 min after Coach's time, `notifyOnCompletion:false`) that runs `node build-dashboards.js` then `update_artifact` for ids `dashboard` and `daily-todo`. Check `list_scheduled_tasks` first; skip if already present. It's background maintenance — do NOT add it to the Daily Menu or `data-agents.js`.
