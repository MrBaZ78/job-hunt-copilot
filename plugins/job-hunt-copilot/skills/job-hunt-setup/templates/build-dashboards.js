...
// Optional last-run stamps for the Run-now cards: a live AGENT_RUNS_JSON env var
// (taskId -> ISO-8601 UTC, set by the refresh task from list_scheduled_tasks) or a
// data-agentruns.js fallback file. Passed through raw and formatted in the USER'S local
// timezone client-side, so it's correct in any country. Absent = the card just omits it.
let RUNS = {};
try {
  if (process.env.AGENT_RUNS_JSON) { RUNS = JSON.parse(process.env.AGENT_RUNS_JSON); }
  else { const rc = fs.readFileSync(path.join(qdir, "data-agentruns.js"), "utf8");
    const w2 = {}; new Function("window", rc)(w2); RUNS = (w2.Q_AGENTRUNS && w2.Q_AGENTRUNS.runs) || {}; }
} catch (e) { RUNS = {}; }
const REFRESH_TASK = "refresh-pinned-dashboards"; // the standard dashboard-refresh maintenance task
...
// Run-now menu: every agent in data-agents.js that carries a scheduled-task id, then the
// dashboard-refresh maintenance task. Each card shows its last-run time when known (raw ISO in `lr`).
...
if (!AGENTS.some(a => a.task === REFRESH_TASK)) AGENTS.push({ ic:"🔄", nm:"Refresh", md:"Rebuilds this dashboard so it shows today's jobs, upgrades and asks.", mt:"", task: REFRESH_TASK });
AGENTS.forEach(a => { a.lr = RUNS[a.task] || ""; });
...
function fmtLocal(iso){try{var d=new Date(iso);if(isNaN(d.getTime()))return "";return d.toLocaleString(undefined,{weekday:"short",day:"numeric",month:"short",hour:"numeric",minute:"2-digit"});}catch(e){return "";}}
function bld(){var applyJ=[],passJ=[],accS=[],rejS=[],askC=[],recA=[],recR=[];Object.keys(ST).forEach(function(k){var v=ST[k];if(k.indexOf("job:")===0){(v==="yes"?applyJ:passJ).push(k.slice(4));}else if(k.indexOf("sug:")===0){(v==="yes"?accS:rejS).push(k.slice(4));}else if(k.indexOf("rec:")===0){(v==="go"?recA:recR).push(k.slice(4));}else if(k.indexOf("ask:")===0&&v==="go"){var a=ASKMAP[k];if(a&&a.cmd)askC.push(a.cmd);}});var p=[];if(applyJ.length)p.push("apply to job"+(applyJ.length>1?"s":"")+" "+applyJ.join(", "));if(passJ.length)p.push("not interested in job"+(passJ.length>1?"s":"")+" "+passJ.join(", "));if(accS.length)p.push("accept CV upgrade"+(accS.length>1?"s":"")+" "+accS.join(", "));if(rejS.length)p.push("reject CV upgrade"+(rejS.length>1?"s":"")+" "+rejS.join(", "));if(recA.length)p.push("accept Coach's change"+(recA.length>1?"s":"")+" #"+recA.join(", #"));if(recR.length)p.push("reject Coach's change"+(recR.length>1?"s":"")+" #"+recR.join(", #"));askC.forEach(function(c){p.push(c);});var n=applyJ.length+passJ.length+accS.length+rejS.length+recA.length+recR.length+askC.length;return {count:n,text:p.length?("From my Daily To-Do, please: "+p.join("; ")+"."):""};}
...
if(HUB_AGENTS.length){sec("▶ Start a helper early");HUB_AGENTS.forEach(function(a){var el=document.createElement("div");el.className="menu-card";el.innerHTML='<div style="font-size:20px;flex:none;width:26px;text-align:center">'+a.ic+'</div><div class="mm"><div class="mn">'+a.nm+'</div><div class="md">'+a.md+'</div><div class="mt">'+(a.mt?"Runs on its own · "+a.mt:"")+'</div>'+(a.lr?'<div class="mt">Last run: '+fmtLocal(a.lr)+'</div>':'')+'</div>';var b=document.createElement("button");b.className="runbtn";b.textContent="▶ Run now";b.onclick=function(){runAgent(a.task,a.nm,b);};el.appendChild(b);root.appendChild(el);});}
...
window.hubQueue=function(k,v){HIDDEN=false;ST[k]=v;save(ST);outbox();};
window.hubDrop=function(k){delete ST[k];save(ST);outbox();};
window.hubGet=function(k){return ST[k];};
outbox();
})();
</script>`; }
