...
## STEP 6 — Finish
Confirm what was built (profile v1, boards chosen, 5 agents scheduled with their times), point them to `Agent-System/progress-dashboard.html` and explain its 4 tabs in one line each (Today = the only tab they must check; Progress; My Profile; The Team). Tell them the first Scout run will land at their chosen hour tomorrow (or offer to trigger Scout now). Explain the simple daily rhythm in plain words: **each day Coach sends one short summary; everything they need to do sits in the To-Do list on the Today tab; they act by just replying to you** (e.g. picks, "not interested in N", "accept #", "go", "show shortlist"). They can type **"what's new?"** any time and you'll read `data-todo.js` and tell them the current list right then. The system never submits, sends, spends, or logs in without them.

**Ongoing (you, the coordinator / "Chief of Staff"):** the To-Do list is the single place the user acts. When they type "what's new?" or "what's on my plate?", read `data-todo.js` and tell them the current items in order with the exact reply for each. When they reply to act (accept / go / show / not interested / a picked role), run the matching action yourself — whichever agent it belongs to — so they only ever talk to you.
