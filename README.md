...
Everything lands on **one pinned tile — the Dashboard** — with **one plain daily summary**, so you act from a single place and never chase five separate agents. Its **Today tab is a Daily Hub**: **▶ Run now** for each helper up top, then today's jobs (**✓ Apply / ✕ Not interested**), CV upgrades (**✓ Accept / ✕ Reject**) and asks — each tap drops one line you paste into the chat. Kept current by a daily refresh. Written in plain, everyday English throughout.
...
4. After setup, act from the pinned **Daily To-Do** panel (tap Apply / Accept / Reject) or just type to Claude — e.g. **"menu"** (open the Daily Menu), **"what's new?"** (see what's waiting), **"show shortlist"**, **"run Scout"**, **"accept all"**, **"go"**. Full list in the plugin README.
